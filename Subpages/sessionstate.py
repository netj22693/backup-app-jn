...
# # Initialize session state for the widget if it doesn't exist
# if 'my_multiselect' not in st.session_state:
#     st.session_state.my_multiselect = ["A", "B", "C", "D"] # Or your default value

# # Define a function to reset the filter
# def reset_multiselect():
#     st.session_state.my_multiselect = ["A", "B", "C", "D"]
...
# st.button("Reset Multiselect", on_click=reset_multiselect)

# Display the selected values
# st.write(f"Selected values: {st.session_state.my_multiselect}")

# Example with st.selectbox
if 'my_selectbox' not in st.session_state:
    st.session_state.my_selectbox = "Option 1" # Default value

def reset_selectbox():
    st.session_state.my_selectbox = "Option 1"

st.selectbox("Choose an option", ["Option 1", "Option 2", "Option 3"], key="my_selectbox")

st.button("Reset Selectbox", on_click=reset_selectbox)

st.write(f"Selected value: {st.session_state.my_selectbox}")
...
if object_from_upload is not None:
    ...
    # try:
        ...
        # Initialize session state for the widget if it doesn't exist
        if 'k_multi_cat' not in st.session_state:
            st.session_state.k_multi_cat = unique_value # Or your default value

        # # Define a function to reset the filter
        def reset_multiselect():
            st.session_state.k_multi_cat = unique_value

        # # Multiselect widget
        # st.multiselect(
        #     "Choose options",
        #     options=unique_value,

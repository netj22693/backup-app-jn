...
if object_from_upload is not None:
    ...
    st.write(min_value_price)
    st.write(max_value_price)
    
    min_fl = float(min_value_price)
    max_fl = float(max_value_price)
    ...
    & (data_table["Price"] <= max_fl)
    & (min_fl <= data_table["Price"]) 
    ]

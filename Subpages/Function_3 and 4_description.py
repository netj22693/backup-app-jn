...
st.write(
    "Simplified version of a case when user is supposed to manually enter details about purchase/order and system to produce an invoice. There is a possibility to produce either XML or JSON - NOT both at the same time due to unique IDs of invoice and order generated by the application. In case that user wants a different file format than was produced, there can be mapping/translation to different format used."
    )
